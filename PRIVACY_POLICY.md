...
Comment Block Helper for Facebook is a Chrome extension that helps users select visible Facebook comment authors, highlight comments that match user-defined keywords, and start Facebook's own block flow for selected profiles.
...
The keyword dictionary is stored with `chrome.storage.sync`, so Chrome may sync it through the user's Google account if Chrome Sync is enabled. Previous keyword data stored locally by older versions may be migrated to `chrome.storage.sync`.
...
Users can remove selected authors by unchecking them on the Facebook page or by reloading/closing the tab. Users can remove keyword dictionary entries from the extension's in-page panel. Users can uninstall the extension from Chrome at any time.
